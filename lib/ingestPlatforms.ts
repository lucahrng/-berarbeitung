...
export async function getMythWeights(admin: any): Promise<Record<string, number>> {
  const { data } = await admin
    .from("candidates")
    .select("myth_id, status")
    .in("status", ["accepted", "rejected"]);

  const counts: Record<string, { accepted: number; rejected: number }> = {};
  for (const row of data ?? []) {
    if (!row.myth_id) continue;
    if (!counts[row.myth_id]) counts[row.myth_id] = { accepted: 0, rejected: 0 };
    if (row.status === "accepted") counts[row.myth_id].accepted++;
    else counts[row.myth_id].rejected++;
  }

  const weights: Record<string, number> = {};
  for (const [mythId, c] of Object.entries(counts)) {
    weights[mythId] = (c.accepted + 1) / (c.accepted + c.rejected + 2);
  }
  return weights;
}

export function applyLearnedWeight(priority: number, weight: number | undefined) {
  const w = weight ?? 0.5;
  return Math.max(1, Math.min(100, Math.round(priority * (0.5 + w))));
}
...
export async function ingestApifyPlatform(opts: {
  ...
  mythWeights?: Record<string, number>;
  mapItem: (item: any) => {
    text: string;
    views: number;
    likes: number;
  ...
  const { admin, myths, platform, actorId, input, mapItem, maxItemsToProcess = 12, mythWeights = {} } = opts;
  let inserted = 0;
  let matched = 0;
