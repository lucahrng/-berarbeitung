// Zentrale Scoring-Logik – wird sowohl beim Ingest als auch (optional) client-seitig
// zur Anzeige/Neuberechnung genutzt. Alles hier ist bewusst nachvollziehbar/transparent
// gehalten, damit Chris "vertraut", warum ein Video priorisiert wurde.
...
export function computePriorityScore(v: CandidateInput) {
  ...
  const score = Math.round(raw * confidenceMultiplier);
  return Math.min(100, Math.max(score, 1));
}
