...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return withCORS(NextResponse.json({ error: "Kein gueltiger JSON-Body mit 'platform' gefunden" }, { status: 400 }));
  }

  const config = PLATFORM_CONFIGS[platform];
  if (!config) {
    return withCORS(NextResponse.json(
      { error: `Unbekannte Plattform '${platform}'. Erlaubt: ${Object.keys(PLATFORM_CONFIGS).join(", ")}` },
      { status: 400 }
    ));
  }
  ...
  try {
    ...
    return withCORS(NextResponse.json({ ok: true, platform, ...result }));
  } catch (e: any) {
    return withCORS(NextResponse.json({ ok: false, platform, error: e.message }, { status: 500 }));
  }
}
