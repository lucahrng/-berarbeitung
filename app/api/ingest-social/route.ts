...
import { ingestApifyPlatform, PLATFORM_CONFIGS, MythRow, getMythWeights } from "@/lib/ingestPlatforms";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await ingestApifyPlatform({
      admin, myths, platform: "x", actorId,
      input: config.buildInput(),
      mapItem: config.mapItem,
      mythWeights,
    });
    return withCORS(NextResponse.json({ ok: true, inserted: result.inserted, errors: [], platformStats: { x: result } }));
  } catch (e: any) {
