...
// Generischer Helfer fuer alle Apify-basierten Plattformen (TikTok, Instagram, X, LinkedIn).
// Jeder Actor liefert leicht andere Feldnamen zurueck - deshalb ein Mapping-Objekt pro Plattform,
// das mit Fallback-Ketten (??) versucht, die gaengigsten Feldnamen der bekannten Actors abzudecken.
...
export async function POST(req: NextRequest) {
  ...
  // ---------- Apify-basierte Plattformen: TikTok, Instagram, X, LinkedIn ----------
  if (process.env.APIFY_TOKEN) {
    // TikTok
    try {
      inserted += await ingestApifyPlatform({
    ...
    // X (Twitter)
    // Actor-Beispiel: apidojo/tweet-scraper - Feldnamen ggf. an den tatsaechlich
    // gewaehlten Actor aus dem Apify Store anpassen (siehe README).
    try {
      inserted += await ingestApifyPlatform({
    ...
    // LinkedIn
    // Actor-Beispiel: harvestapi/linkedin-post-search - Feldnamen ggf. an den
    // tatsaechlich gewaehlten Actor aus dem Apify Store anpassen (siehe README).
    try {
      inserted += await ingestApifyPlatform({
