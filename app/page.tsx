...
const PLATFORM_META: Record<string, { label: string; icon: any; color: string }> = {
  ...
  x: { label: "X", icon: Twitter, color: "#8A8378" },
...
export default function Page() {
  ...
  return (
    <div className="min-h-screen pb-16">
      <header className="border-b border-black/10 sticky top-0 bg-ink/95 backdrop-blur z-20">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 pt-5">
        <div className="flex items-center gap-1 mb-4 bg-black/5 rounded-lg p-1 w-fit">
          {[
          ...
          {["all", "youtube", "tiktok", "instagram", "x", "linkedin"].map((p) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-full text-xs border transition ${
                platformFilter === p ? "bg-paper text-ink border-paper" : "border-black/15 text-paper/60 hover:border-black/30"
              }`}
            >
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="relative">
              <select
                ...
                className="appearance-none bg-black/5 border border-black/15 rounded-full text-xs pl-3 pr-7 py-1.5 outline-none cursor-pointer"
              >
            ...
            {tab === "accepted" && acceptedList.length > 0 && (
              <button
                onClick={exportCSV}
                className="flex items-center gap-1.5 text-xs bg-black/5 border border-black/15 rounded-full px-3 py-1.5 hover:bg-black/10 transition"
              >
            ...
            {filtered.map((c) => {
              ...
              return (
                <div
                  ...
                  className="group bg-black/[0.03] hover:bg-black/[0.06] border border-black/10 rounded-xl p-4 cursor-pointer transition"
                >
                    ...
                    {tab === "inbox" && (
                      <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                        <button
                          ...
                          className="w-7 h-7 rounded-full border border-black/15 flex items-center justify-center hover:bg-black/10 hover:border-black/30 transition"
                        >
                          <X size={13} />
                        </button>
                        <button
...
function DetailModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40 flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="bg-[#FBF7EF] border border-black/10 w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto">
        <div className="p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={onClose} className="w-7 h-7 rounded-full hover:bg-black/10 flex items-center justify-center">
              <X size={15} />
            </button>
          </div>
          ...
          {myth && (
            <>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {myth.sources.map((s) => (
                  <a key={s.name} href={s.url} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[11px] bg-black/5 hover:bg-black/10 border border-black/10 rounded-full px-2.5 py-1 transition">
              ...
              {!showCorrection ? (
                ...
                <div className="bg-black/[0.04] border border-black/10 rounded-lg p-4 mb-3 text-sm leading-relaxed space-y-3">
                  <div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs bg-black/10 hover:bg-black/15 rounded-full px-3 py-1.5 transition"
                  >
          ...
          {candidate.status === "pending" && (
            <div className="flex gap-2">
              <button onClick={() => onDecide(candidate, "rejected")} className="flex-1 flex items-center justify-center gap-1.5 border border-black/15 rounded-lg py-2 text-sm hover:bg-black/5 transition">
                <X size={14} /> Ablehnen
              </button>
