...
export default function AdminPage() {
  ...
  async function callEndpoint(endpoint: string, label: string, body?: Record<string, unknown>) {
    try {
      ...
      addLog({
        ...
        text: `${res.ok ? "✓" : "✗"} ${label} (${res.status})\n${JSON.stringify(data, null, 2)}`,
        ok: res.ok,
      });
    } catch (e: any) {
      addLog({ time: new Date().toLocaleTimeString("de-DE"), text: `✗ ${label}: ${e.message}`, ok: false });
    }
  }

  async function runSequence(groupLabel: string, steps: { endpoint: string; label: string; body?: Record<string, unknown> }[]) {
    if (!secret.trim()) {
      addLog({ time: new Date().toLocaleTimeString("de-DE"), text: "Bitte Ingest-Secret eingeben.", ok: false });
      return;
    }
    setLoading(groupLabel);
    for (const step of steps) {
      await callEndpoint(step.endpoint, step.label, step.body);
    }
    setLoading(null);
  }
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 max-w-2xl mx-auto">
      ...
      <div className="border border-white/10 rounded-xl p-4 mb-3">
        ...
        <button
          onClick={() => runSequence("embed-myths", [{ endpoint: "embed-myths", label: "Embeddings" }])}
      ...
      <div className="border border-white/10 rounded-xl p-4 mb-3">
        ...
        <p className="text-xs text-paper/55 mb-3">Läuft nacheinander durch, meist schnell und zuverlässig.</p>
        <button
          onClick={() =>
            runSequence("fast", [
              { endpoint: "ingest", label: "YouTube" },
              { endpoint: "ingest-social", label: "Instagram", body: { platform: "instagram" } },
              { endpoint: "ingest-social", label: "TikTok", body: { platform: "tiktok" } },
            ])
          }
          disabled={loading === "fast"}
          className="w-full bg-accept rounded-lg py-2.5 text-sm font-semibold disabled:opacity-50"
        >
          {loading === "fast" ? "läuft..." : "YouTube + Instagram + TikTok einsammeln"}
        </button>
      </div>

      <div className="border border-white/10 rounded-xl p-4 mb-4">
        <h2 className="text-sm font-semibold mb-1">3. LinkedIn + X</h2>
        <p className="text-xs text-paper/55 mb-3">Oft langsamer (v.a. LinkedIn) – deshalb getrennt.</p>
        <button
          onClick={() =>
            runSequence("slow", [
              { endpoint: "ingest-social", label: "LinkedIn", body: { platform: "linkedin" } },
              { endpoint: "ingest-social", label: "X", body: { platform: "x" } },
            ])
          }
        ...
        >
          {loading === "slow" ? "läuft..." : "LinkedIn + X einsammeln"}
        </button>
      </div>
